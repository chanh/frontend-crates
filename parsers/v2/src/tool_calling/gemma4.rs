...
pub(crate) fn gemma4_scanner(tools: &[Tool]) -> WrappedBlockScanner<Gemma4InvokeEmitter> {
    WrappedBlockScanner::new(
        WrappedBlockSpec {
            ...
            // The string delimiter is held back too: a `<|"|>` split across a
            // chunk boundary must not be released as text, or the value it opens
...
/// v1 probe (complete, or a `call:NAME{` still filling in).
///
...
fn opens_bare_call(text: &str, at: usize) -> bool {
    is_call_prefix_boundary(text, at) && detect_tool_call_start_gemma4(&text[at..])
}
...
/// arrives, `detect_tool_call_start_gemma4` recognizes the opener directly.
